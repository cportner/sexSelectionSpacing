...
//     foreach where in "urban"  {
        ...
        forvalues spell = 2/4 {
			...
			if `spell' == 2 {
				loc pattern = "solid shortdash"
			}
			if `spell' == 3 {
				loc pattern = "solid longdash shortdash"
			}
			if `spell' == 4 {
				loc pattern = "solid longdash dash shortdash"
			}
...
//                local label "order(1 "1 daughter" 2 "1 son") cols(1) rows(2)  " 
                local title "{bf:Second spell}"
				...
				// local label "order(1 "2 daughters" 2 "1 son/" "1 daughter" 3 "2 sons") rows(4)  span "
				//local label "order(1 "2 daughters" 2 "1 son/1 daughter" 3 "2 sons") cols(2) rows(2) "
                local title "{bf:Third spell}"
                local bi_title1 ""
                local bi_title2 ""
                local sr_title1 "" 
                local sr_title2 ""
...
//                 }
                ...
                // local label "order(1 "3 daughters" 2 "1 son/" "2 daughters" 3 "2 sons/" "1 daughter" 4 "3 sons") rows(4) span "
				// local label "order(1 "3 daughters" 2 "1 son/2 daughters" 3 "2 sons/1 daughter" 4 "3 sons") cols(2) rows(2) "
                local title "{bf:Fourth spell}"
                local bi_title1 ""
                local bi_title2 ""
                local sr_title1 "" 
                local sr_title2 ""
...
//             graph export `figures'/bs_spell`spell'_`educ'_`where'_all.eps, replace fontface(Palatino) 
        ...
        gr combine interval pct any  , ///
            col(1) xcommon ysize(9) xsize(7.5) imargin(0 0 3 0) iscale(*1.2) // Used to be xsize(6.5)
