...
set scheme s1mono
preserve
collapse educ_none educ_1_7 educ_8_up , by(cohort place_type)
...
// Urban
graph twoway rarea null line_0 cohort if urban, color(gs15%30)  lwidth(none) ///
    || rarea line_0 line_1 cohort if urban, color(gs8%30) lwidth(none) ///
    || rarea line_1 one cohort if urban, color(gs3%30) lwidth(none) ///
    || , legend(label(1 "No education") label(2 "1 to 7 years") label(3 "8 or more years") ring(0) position(8) col(1)) ///
        plotregion(margin(zero)) xlabel(1930(10)1990) 

graph export `figures'/educ_over_time_urban.eps, replace fontface(Palatino)
