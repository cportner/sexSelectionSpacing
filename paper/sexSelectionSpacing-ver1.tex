...
Although it might be tempting to simply examine how observed
spacing has changed over time this is problematic.

Possible ways of doing this:
- Only closed spells (where a birth is observed)
    - can be split by the outcome (ending birth is either male or female)
    - can be split by the sex composition of prior children
- All spells (whether closed or not)
    - can only be split by the sex composition of prior children
    
Problems:
- Using closed spells only ignore that those families with the strongest
  son preference are also likely to have the longest spacing and therefore be 
  less likely to show up as a complete spell.
- Because of the potential for changes in the use of sex selection within a spell
  it is possible that parents go through multiple sex-selective abortions and still
  end up with a girl
- Some open spells may not result in a birth (childbearing finished with prior birth),
  which is especially a problem for higher parities where sex composition of
  prior children has a large effect on the probability of parity progression.
  
Need a method that accounts for censoring.

In addition to accounting for censoring, the empirical method must be able 
to capture the three main implications of the availability of sex-selective 
abortions combined with son preference:
...
sex determination.
This means an empirical model that can jointly estimate 
...
the period between births (or from marriage to first birth for the first spell).
...
The standard proportionality of hazards assumption does not work here.
First, even in the absense of sex selection, it is unlikely that the shape of
hazard functions are identical across groups.
[sex composition]
