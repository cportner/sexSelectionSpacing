...
I divide a woman's reproductive life into one or more spells.
Each spell covers the period from one birth, or marriage for the first spell,
to the following birth; I define how the duration is measured in more 
detail in Section \ref{sec:data}.
A basic problem is that some spells' durations are censored, either
because the survey took place before the end birth for the spell or because
the couple has finished childbearing.
The standard approach, which I follow here, is to use a hazard models to 
address the censoring of spells.

Prior research on birth spacing have used hazard models where the exit 
state was the birth of a child. 
...
sex selection. 
To capture that the sex of children are no longer a random event, I instead
use a competing risk framework with two possible exit states: either a boy 
or a girl is born.%
\footnote{
...
Proportional hazard models, where covariate have an multiplicative 
...
The problem here is that a proportional hazard model does not allow covariates
to have different sized effects at different times within a spell and therefore
cannot capture differences in the shape of the hazard functions between 
different groups, and if the proportionality assumption does not hold 
the result is a potentially substantial bias in estimates.
It is unlikely---even in the absence of prenatal sex 
determination---that the effect of, for example, the sex composition
of prior births have the same effect throughout the entire spell.
[son preference and parity progression also leads to differences!]
Furthermore, any bias from the proportionality assumption is likely 
exacerbated by the introduction of prenatal sex determination.
First, sex-selective abortions affects birth spacing and its use
vary across groups.
Second, the use of sex selection may vary within a spell, which means 
that the effect of covariate may vary within the spell as well.
Finally, the use of a non-proportional specification, together with a 
flexible baseline hazard, also mitigates any potential effects of 
unobserved heterogeneity \citep{Dolton1995}.
...
The $Z$ set of explanatory variable and the interactions between them 
...
Hence, if the data are transformed so the unit of analysis is spell unit, in 
...
The main downside is that direct interpretation of the estimated 
coefficients is challenging because of the competing risk setup.
...
Second, a, for example, positive coefficient does not necessarily imply that
a higher variable value is associated with a higher event probability because the 
...
the probability of not having had a birth yet by spell duration.
