...
% This is possible because the hazard model approach uses both censored and 
% uncensored observations, which, in turn, allows me to predict both how many 
% women will have a next birth and the final sex ratio for that parity, under the 
% assumption that the patterns of spacing and use of sex selection remain 
% the same within a spell.
...
% Two factors determine the differences in the predicted sex ratio and number 
% of abortions between the two models.
% First, how many women have censored spells but will have a birth after the survey.
% Second, how much use of sex selection changes within a spell.
% % If there are few censored spells or the women with the censored spells have low likelihoods
% % of having a next birth the two models produce similar predicted sex ratios and 
% % number of abortions.
% % If there is little change in the use of sex-selective abortions over a spell then the
% % predicted sex ratio is going to be similar for the two models, although the predicted
% % number of abortions will differ depending on how many censored observations there are
% % and how likely they are to exit with a birth.
% Consider four possible combinations of the two factors.
% First, there are few censored birth spells and use of sex selection is constant over
% the spell.
% In that case, the predictions will be similar across the two models.
% Second, there are many censored birth spells and use of sex selection is constant over
% the spell.
% The predicted sex ratios will be similar, but the simple model will predict a too low
% number of abortions per woman and fertility will appear to be falling faster than it is.
% Third, there are few censored birth spells and use of sex selection changes substantially
% with spell length.
% The sex ratios again are close to each other in the two models, but the estimated use of
% abortions will be biased in the simple model because it cannot account for the changes
% in use of sex selection over the spell.
% Finally, there are many censored birth spells and use of sex selection changes substantially
% with spell length.
% In this case the predicted sex ratio based on the simple model will be biased.
% Specifically, if use of sex selection declines over the spell, the simple model 
% will predict a higher final sex ratio than the hazard model.
% Similarly, the predicted number of abortions will also be biased. 
% With sex selection and falling fertility censored spells become more and more likely,
% but how much this matter is an empirical question.

% To determine the predicted number of abortions I use the natural sex ratio.
% On average, 105 boys should be born per 100 girls born, which corresponds exactly
% with what we observe for first births in India as seen above.
% Hence, if we observe $b$ boys, then we should expect to see $b\frac{100}{105}$ girls.
% If we observe $g$ girls the number of abortions is therefore $b\frac{100}{105}-g$.
% The standard approach is to use the observed sex ratio for a given parity and 
% calculate the number of abortions using this formula.
% For the hazard model it is, however, possible to calculate the number of predicted
% abortions for each time period based on the predicted sex ratio and the 
% number of women predicted to exit with a birth in that period using the same formula.
% Summing across the entire period covered by the hazard model provides a predicted total 
% number of abortions.%
% \footnote{
% There is no difference between the two method in the predicted number of abortions for 
% those women for whom we observe the subsequent birth.
% }

% [NEED TO WORK THROUGH A MATH EXAMPLE OF THE FIRST FACTOR: IS THAT REALLY CORRECT?]
% period 0: 100 births 65 boys 35 girls 26.90476191
% period 1: 100 births 60 boys 40 girls 17.14285714
% period 2: 100 births 55 boys 45 girls 7.380952381
%                     180 boys 120 girls abortions: 51.42857143
% 
% period 0: 1000 women: 512 boys 400 girls , 88 abortions  87.61904762
% period 1:   98 women: 50 boys 42 girls, 6 Abortions  5.619047619
% period 1:    6 women: 3.073 boys 2.927 girls 0 abortions -0.000333333
%                     562 boys 442 girls    93.23809524 93.2377619 93.23776191
% 
% [HOW TO CALCULATE NUMBER OF ABORTIONS HERE]
%
% In addition, the standard model does not provide any information about abortions
% for those women for whom we do not observe a birth.
% In contrast, the hazard model can be used to predict the number of abortions even for 
% those women who have not yet had a birth at the parity of interest.
% This number is the sum of two set of predictions.
% First, the number of abortions that are predicted have occurred up to the time of the 
% survey for women without a given parity birth.
% Second, we can predict how many additional abortions will take in the future for these
% women from the time of censoring until the end of the spell period used for the hazard
% model.
% This follows directly from the standard feature of hazard models where one can predict
% the likelihood of future exits for those censored.
% To the extent that people change their use of sex-selective abortions within a spell
% the two numbers will be different.
%
% [TRYING TO WORK THROUGH THE SET-UP HERE]
% 
% For each women who had one birth we follow her until 24 quarters (6 years) after the 
% birth of her first child. 
% Contrary to the standard logit model we can predict two things: How likely it is that she
% will have her second child during a particular quarter and what the sex ratio will be in 
% that quarter.
% In other words, by the end of the 24 quarter period we will know the probability of her
% having had a child, which is what is shown by the survival curves in the graphs above,
% and expected sex ratio for all women like her.
% So, one women might have 73 percent probability of giving birth to a second child within
% the 24 quarters with 58.3 percent probability of a boy and a 41.7 percent probability
% of a girl, whereas another might only have a 29 percent probability of giving birth to a 
% second child with a 55.4 percent probability of a son and a 44.6 percent probability of a
% girl.
% 
% Summing across all women with specific characteristics, like being an urban woman with
% a girl as her first child, we can then predict how many children will be born to these
% women in the 6 years after the birth of their first child, together with information on
% how many abortions will have taken place and the final sex ratio for those second-born
% children.
% 
% The advantage of this is that it is not reliant just on observed births.
% To the extent that the probability of using sex-selective abortions changes with time since
% the last birth the results from the hazard model prediction will be different from the
% Logit model prediction (which is just the sex ratio for birth observed in the 6 year 
% period).
% Furthermore, even if the probability of sex-selective abortions does not change substantially 
% with duration since last birth, the hazard model provides an estimate of how many births
% there will be and what the final sex ratio for a given parity birth will be.
% The more censored observations there are the bigger the difference between the logit and
% the hazard models are going to be.
% This is important because that those numbers are an input into determining what the
% population sex ratio will be.

% [is there a way of understanding the difference between censored and uncensored women
% using the hazard model?
% One option would be to run everybody until they are censored and see if that fits with
% the logit numbers.
% that should presumably match the logit results except for picking up differences in use
% of sex-selective abortions. 
% This means that the births should be the same for the two methods.
% 
% For those censored we could then run the rest of the time. 
% This would require that we started from one woman again, instead of continuing from the
% censoring left off.
...
%
% We begin with women who had a girl as their first child.
% Table \ref{tab:compare2} shows the number of women who enter the second spell,
% the number of births by the end of the six year period following the first birth,  
% observed in the case of the logit model and predicted in the case of the hazard model, 
% predicted number of abortions, and the sex ratio.%
% \footnote{
% The two latter obviously captures the same information and is both presented for
...
% [CLOSING ARGUMENTS ON COMPARISON?]
% [logit will tend to underestimate when sex-selective abortions is spreading/increasing
% and overestimate when sex-selective abortions is declining.]

% Urban
% 0.566585492 hazard sex ratio for 1985
% 0.592900726 hazard sex ratio for 1995
% 
% 0.554402294 logit sex ratio for 1985
% 0.593989533 logit sex ratio for 1995
% 
% Rural
% 0.544119243 hazard sex ratio for 1985
% 0.579341591 hazard sex ratio for 1995
% 
% 0.530621491 logit sex ratio for 1985
% 0.573022963 logit sex ratio for 1995
...
% I begin with women who had a girl as their first child.
% For rural women in the 1985--1994 period relatively few censored women are
% predicted to go on to have a second birth (only 5 percent of the starting population
% is predicted to have a second birth and have not yet had one).
% In addition, there is only some evidence of sex-selective abortions.
% Hence, the predicted sex ratios for the simple and hazard models are close. 
% There are slightly more abortions for the hazard model, but the abortion rates (number
% of abortions per birth) are fairly similar between the observed births and those predicted
% to have a birth later.
% 
% Urban women in the same time period follow a similar pattern with relatively few 
% censored women predicted to move on to a second birth (around 2 percent of starting 
% population), but the abortion rate for those additional births is 8.1 abortions per 100 
% births, whereas it is only 6.7 for the observed births, as we would expect from the 
% increasing probability of having a son with increasing spell length seen in panel (b) in 
% Figure \ref{fig:results_spell2_high_urban}.
% Despite this the resulting difference in sex ratios is
% relatively small because of the small number of censored women predicted to exit with a
% birth.
% 
% 
% Not surprisingly there are substantially more censored observations for the 1995--2006
% period.
% For urban women about 19 percent of the starting population are predicted to have a
% second birth but have not yet had one and for rural women it is 23 percent.
% This allows for potentially larger differences in predicted sex ratios and number of
% abortions.
% Despite the large number of censored observations for rural women there is relatively
% little difference in the predicted sex ratio.
% The more interesting difference is for urban women in the 1995--2006 period.
% The hazard model predicts 100 more abortions by the end of the spell, but the simple 
% model \emph{overestimates} the final sex ratio.
% The simple model predicts 143.5 boys and the hazard model 141.3 boys per 100 girls.
% This is consistent with the very high probability of having a son early on in the spell
% followed by a somewhat lower probability for later births seen in panel (c) in 
% Figure \ref{fig:results_spell2_high_urban}.
% 
% 
% 
% The lower two panels of Table \ref{tab:compare2} presents the predicted outcomes for urban
% and rural women for the third spell, split by the sex composition of their first two 
% children and when the spell began.%
% \footnote{
% There is no evidence of sex-selective abortions and very few births for women with two boys 
% and those predictions are therefore not presented.
% }
% For urban women with two girls in the 1985--1994 period there are only 77 more births for 
% the hazard model, but the difference in number of abortions is 15 with the hazard model 
% predicting 266 abortions and the simple model 251.
% The resulting sex ratio is 175.3 for the hazard model versus ``only'' 170.0 for
% the simple model.
% In other words, it appears that the simple model underestimates the spread of the use of
% sex-selective abortions during the expansion of access to prenatal sex determination 
% for this group of urban women.
% As discussed in the paper, use of sex selection for urban women with two girls appears to  
% have fallen from the 1985--1994 period to the 1995--2006 period, but this is mainly an 
% indication that sex-selective abortions are being used more during the spell to second 
% birth for this group of women for the 1995--2006 period.
% The result is fewer women with two girls for their first two births and those who do are 
% substantially less likely to have a third birth than they were before.
% 
% For rural women the main difference between the two models is for those with one boy
% and one girl in the 1995--2006 period.
% Only about 60\% of the birth predicted by the hazard model are observed in the simple
% model, leading to substantial censoring.
% Furthermore, use of sex-selective abortions is predicted to be substantial for those
% remaining births resulting in a sex ratio of 129.8 for the hazard model and 124.6 for
% the simple model. 
% As discussed in the paper, this case is the exception to the rule that sex selection
% does not appear to be used by families that already have one son.
% Presumably the use by rural women in this case is an insurance against mortality. 
