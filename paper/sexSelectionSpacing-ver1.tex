...
accounts for how sex selection increases both the likelihood of a son
and the spacing between births. 
Using four rounds of India's National
Family and Health Surveys, I first show that, except for first births,
the spacing between births increased substantially over the last four
decades. 
Birth spacing increased the most for women most likely to use
sex selection; specifically, well-educated women with no boys now
exhibit significantly longer spacing and more male-biased sex ratios
than those with boys. 
...
Second, the increase in duration is substantially larger for the groups of women 
who are most likely to use sex selection.
...
For the first spell I set censoring at 72 months (six years) after marriage,
while for the second and third spells censoring is at 72 months and 
for the fourth spell 57 months after a birth can occur.
With nine month between the previous birth and the beginning of
the next spell, the maximum durations between births I consider are
6.75 years for the second and third spells and 5.5 years for the 
fourth spell.
...
(measured as the change in the survival curve; in other words, there is 
40\% chance that she does not have a birth by the end of this spell).
...
however, can tell us whether sex selection vary \emph{within}
...
economies of scale in childbearing and rearing that would be lost 
if children were born too far apart \citep[p 374]{schultz97}. 
In this case we should observe the predicted sex ratio decline
with spell length.

Second, if the degree of son preference vary across women within 
the sample but behavior is not affected by duration from the 
...
of a girl relative to after the birth of a boy when prenatal sex selection is 
available.
I introduce a new method that simultaneously account for spacing between
births and the potential use of sex selection and apply the method
to over four decades of data from India's NFHS.
...
The situations where there are elevated sex ratios for women without
education---which, by itself, should indicate use of sex selection---the
spacing patterns are too close to the ones without sex selection that 
it is difficult to conclude that the changes in sex ratios indicate
sex selection rather than normal variation or problems with recall errors.
...
These changes are likely due to increased use of sex selection 
rather than changes in son preference as shown by the increasingly
significant deviations in the sex ratio from the natural sex ratio.
This also suggests that banning prenatal sex determination has 
not been able to reverse the use, although it is possible that it
has slowed the rate of increase.
...
in the variation.
The end result is that median spacing from marriage to first birth
...
to change within the spell.
This is important because the use of sex selection is likely to vary
within a spell, either because preferences for spacing between births
that is not too long override the preference for sons or because
of selection where the women who remain in a spell are those with
the strongest son preference who have been through multiple 
pregnancies that were aborted because the fetus was female.
Because these move in opposite directions it is difficult to 
establish the extent to which they affect the results.
Both are, however, clearly present as can be seen from the
cases where there is either a decline in sex ratio with spell
duration or a substantial increase with spell duration.
The first case is more likely at lower parities where there
is still less pressure to ensure a son, which allows any
preference for shorter spacing to play more of a role.
The second is on the other hand more likely to have an effect
for higher parities where the desire to limit further
childbearing and ensure a son dominates any preferences for
shorter spacing.
...
Although it is not possible to directly observe whether the
unequal sex ratio and the longer spacing patterns are caused
by sex selection the results are consistent with a general
increase in sex selection over time.
The increase in apparent use of sex selection is mainly found
among women with more education and more so in urban than in
rural areas, although the differences between urban and rural
areas are relatively minor.
Compared to previous research one result stands out.
It appears that the use of sex selection is spreading to 
women with lower levels of education, so that even women
with one to seven years of education show unequal sex ratios
...
overstated exactly because of an increased use of sex selection and
...
will happen.
The facts that biased sex ratio occur at earlier spells over
time and starting to show up for women with less education 
